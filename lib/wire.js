// dsh-wf 线协议描述（单一来源）
// 职责：领域记录 schema + 远程调用描述（invocations/descriptors），宿主 typert.host.js 与客户端
...
// 导出：META_SCHEMA / BODY_SCHEMA / GLOBAL_SCHEMA / WF_INVOCATIONS
import { z } from 'zod'

// ---------- 领域记录 schema（与 storage-domain 持久边界共用；domain.js 复用） ----------
...
// body 宽松（Element 30+ 字段与前端注册表重复，持久边界只保形状；语义清洗走 sanitizeElements）
export const BODY_SCHEMA = z.object({
  schemaVersion: z.number().int().nonnegative(),
  elements: z.array(z.record(z.string(), z.unknown())).default([]),
}).passthrough()

export const GLOBAL_SCHEMA = z.object({
  lastCanvasId: z.string().nullable(),
})
