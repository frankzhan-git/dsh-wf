...
- **批量调整边对齐吸附**：多选外框四边拖动时，移动边与「非选中元素」边缘对齐吸附（容差 6/zoom，与 move/resize 一致），命中时全部选中控件按吸附修正量统一偏移，并显示吸附虚线
- **修复「光标已显示可拖但点击变选中」**：元素边带命中判定移到 hitTest 之前（对全部元素从上层遍历）——鼠标在元素边缘**外侧 ±8px**（光标 ew/ns 区域）点击现在直接触发 resize，不再落入空白误触发框选；右下角区域限界（14px 内 + 外侧 8px），修复元素外无限延伸误命中
- 验证：verify-interactions **111 断言全绿**（新增批量吸附/吸附容差/边缘外侧点击/右下角外侧/空白回落断言）
...
- **移除多选外框四角等比手柄**：批量调整统一为**上下左右四边**横向/纵向（幅度与虚线框移动量一致）；四角按 r > b > l > t 归边处理
- **修复四边批量不生效的根因**：多选外框边带判定移到**元素命中之前**——元素紧贴外框边缘时，点「虚线框」不再被单元素命中抢走（此前点贴框元素的上/下边会变成调整单个元素）
- **边带加宽**：单选控件边带 6px → 8px（逻辑坐标），更容易命中
- 验证：verify-interactions **102 断言全绿**（新增「边带优先于元素命中」回归 + 角归边断言）

## v2.2.0（2026）

### 交互与画布操作

- **README 产品截图**：新增 `view.png` 截图展示
- **画布点击失焦**：点击画布任意位置，右侧控件设置表单立即失焦（受控组件 value 已实时同步，不丢数据）
- **预览浮窗 resize**：JSONL / 语义预览浮窗支持右下角拖拽调整大小（最小尺寸 = 当前大小，即拖拽起点）
- **控件四边 resize**：除右下角手柄外，支持按住控件上下左右四边横向/纵向改尺寸（左/上边拖动时对侧锚定、最小尺寸与页面边界钳制、移动边对齐吸附与原行为一致）
- **多选外框四边 resize**：框选多选后的虚线外框支持上下左右四边批量调整——每个控件的调整幅度与外框移动量保持一致（线性，非等比）；四角等比缩放保留
- **框选稳定**：完全包含语义保持并由测试强化（含多选外框边手柄命中/角优先/空白回落）
- 验证：verify-interactions 98+ 断言全绿（新增四边 resize / 组四边 resize / 边命中决策 4 组）
