# 更新日志

## v2.2.8（2026）

### 修复：下层控件边带/右下角截胡上层目标控件的拖动

**症状**：目标控件 A 下方有其他控件 B 且 A 覆盖 B 的部分边缘时，拖 A 若鼠标恰好落在 B 的四个边之一（或右下角）——A 拖不动，反而触发对 B 的边拖拽/改尺寸。

**根因**：`decidePointerDown` 中「元素边带优先于命中」——`hitEdgeOfAny` 从上层到下层遍历**全部**元素找边带命中，鼠标在 A 内部但位于 B 的边带/角上时，B 先被返回 → resize B。边带命中没有考虑**被上层元素覆盖**的位置：下层元素的边缘在视觉上已被 A 挡住，不应可操作。

**修复**：决策顺序改为「**元素内部命中优先于一切边带**」——

1. `hitTest` 命中（最上层覆盖该点的元素）→ 由该元素决定：自身边带/角 → 改尺寸；否则 → 选中/移动（目标控件 A 正常拖动）
2. 仅空白处（未被任何元素覆盖）→ 元素边缘外侧 ±8/zoom（含右下角外侧双向）→ resize（v2.2.3「边缘外侧点击不落入框选」语义保留）

**验证**：新增 4 条回归（B 上边缘带被 A 覆盖 / B 右下角被 A 覆盖 → 拖 A；B 自身边缘外侧未被覆盖 → 仍 resize B；多层边带重合 → 顶层优先）；11 套验证全绿。

## v2.2.7（2026）

### 修复：复制粘贴后画布「假死」——副本 id 与既有元素冲突

**症状**：刷新浏览器后打开已有画布，复制粘贴一个控件/页面，画布像是卡死——粘贴出的副本点不动、拖不走、删不掉；关闭后再打开数据错乱，只能整页刷新。

**根因**：元素 id 由模块级序号 `seq` 生成（`e1, e2, …`），`seq` 只在页面生命周期内递增。浏览器刷新后 `seq` 归零，而画布元素 id 仍从 `e1` 起——此时粘贴（`buildPaste` + `nextId()`）生成的副本 id 与画布现有元素**完全重复**。由此产生连锁错误：

- 按 id 寻址命中原元素：副本无法独立选中/拖动/删除（用户感知为「画布假死」）
- React 渲染 key 重复：原件与副本视觉叠置、状态串扰
- 自动保存按 id 覆盖（`set[id] = e`）：原元素被副本替换，**数据丢失**

**修复（双保险）**：

- **主修复 `reserveSeqs`**：`model.js` 新增 id 序列恢复函数；所有元素载入点（打开画布 `initLast`、异步恢复 `restoreLast`、历史列表载入 `loadCanvas`）载入后立即推进 `seq` 到已载入元素最大 id 之后——新生成的 id 永不与载入元素冲突
- **防御 `buildPaste(existing)`**：`buildPaste` 新增可选 `existing`（画布现有 id 集合），分配 id 时逐次跳过冲突——即使某条载入路径漏了 `reserveSeqs`，副本 id 也保持全局唯一
- 验证：新增 `verify-paste-id.mjs`（独立进程模拟「刷新后 seq=0 + 载入旧画布」场景，8 断言：冲突防御 / seq 推进 / 完整链 / 副本可独立寻址）；11 套验证全绿

## v2.2.6（2026）

### 预览浮窗与滚轮行为

- **JSONL 内容撑满弹窗**：浮窗默认高度固定 380px，`.wf-jsonl` 移除 `max-height: 320px` 限制，内容区 flex 撑满面板剩余空间（可右下角拖拽调整）
- **滚轮 = 画布滚动（同滚动条效果）**：普通滚轮上下滚动画布（Shift+滚轮水平滚动），滚轮增量按行/像素归一化并换算逻辑坐标；方向与滚动条一致
- **Ctrl/⌘+滚轮 = 缩放**：原滚轮缩放行为迁移到按住 Ctrl（或 ⌘）时触发（中心锚点缩放不变）
- 验证：构建成功 + 10 套验证全绿

## v2.2.5（2026）

### 修正：批量调整吸附「高度突变/位置抖动」

- **根因**：吸附修正基于「未吸附的原始累计位置」每帧重复叠加——第 1 帧吸附到目标后，鼠标不动/微移时修正量被反复应用（位置持续偏移、高度持续突变，直至最小尺寸钳制）
- **修复**：吸附修正并入「实际应用累计」（appX/appY = 原始累计 + 吸附修正），本帧增量 = 应用累计差；hooks 的 `lastDx/lastDy` 改用应用累计更新 → 吸附锁定期间增量为 0（面板钉住目标边缘），移出容差恢复跟随
- **四边逐一验证**（t/r/b 多帧吸附回归测试）：第 1 帧吸附到位 → 第 2 帧鼠标不动位置保持 → 容差内微移锁定 → 移出容差恢复跟随
- 验证：verify-interactions **119 断言全绿**（新增 8 条多帧吸附回归）

## v2.2.4（2026）

### 修正：预览浮窗 resize 行为

- **修复「resize 时左侧边在扩大」**：浮窗定位从 `right: 8px`（右对齐锚定，宽度增大时左边缘外扩、手柄不动）改为**窗口居中**（`left/top: 50%` + `translate(-50%,-50%)`，中心锚定）
- **resize 按鼠标位移双倍增长**：居中锚定下宽/高对称扩展，右下角手柄跟随鼠标——鼠标横移 dx → 宽度 +2dx（左右各扩 dx），纵移 dy → 高度 +2dy（上下各扩 dy）
- 最小尺寸 = 拖拽起点（当前大小）不变
- 验证：构建成功 + 10 套验证全绿

## v2.2.3（2026）

### 优化：批量调整吸附 + 边拖拽可点性

- **批量调整边对齐吸附**：多选外框四边拖动时，移动边与「非选中元素」边缘对齐吸附（容差 6/zoom，与 move/resize 一致），命中时全部选中控件按吸附修正量统一偏移，并显示吸附虚线
- **修复「光标已显示可拖但点击变选中」**：元素边带命中判定移到 hitTest 之前（对全部元素从上层遍历）——鼠标在元素边缘**外侧 ±8px**（光标 ew/ns 区域）点击现在直接触发 resize，不再落入空白误触发框选；右下角区域限界（14px 内 + 外侧 8px），修复元素外无限延伸误命中
- 验证：verify-interactions **111 断言全绿**（新增批量吸附/吸附容差/边缘外侧点击/右下角外侧/空白回落断言）

## v2.2.2（2026）

### 修正：批量调整幅度与光标

- **修复批量调整「放大」**：多选外框边拖动时每个控件的调整量改按**本帧位移**计算（累计位移 - lastDx/lastDy）——此前拖动中 elements 每帧更新后累计位移被重复累加（旧 bug 同 computeMove 曾修复的多帧重复累加）；新增多帧回归测试（第 2 帧只加本帧增量，110+10=120 而非 130）
- **四边光标修复**：`.wf-edge` 移除 `pointer-events: none`——此前 CSS cursor 在 pointer-events:none 元素上不生效（事件穿透时光标由下层决定），上下边现在显示 `ns-resize`（上下箭头）、左右边显示 `ew-resize`（左右箭头）
- draw 模式四边手柄光标统一为 crosshair；空格平移模式覆盖为 grab/grabbing
- 验证：verify-interactions **104 断言全绿**（新增多帧增量回归 2 条）

## v2.2.1（2026）

### 修正：四边 resize 语义与可用性

- **移除多选外框四角等比手柄**：批量调整统一为**上下左右四边**横向/纵向（幅度与虚线框移动量一致）；四角按 r > b > l > t 归边处理
- **修复四边批量不生效的根因**：多选外框边带判定移到**元素命中之前**——元素紧贴外框边缘时，点「虚线框」不再被单元素命中抢走（此前点贴框元素的上/下边会变成调整单个元素）
- **边带加宽**：单选控件边带 6px → 8px（逻辑坐标），更容易命中
- 验证：verify-interactions **102 断言全绿**（新增「边带优先于元素命中」回归 + 角归边断言）

## v2.2.0（2026）

### 交互与画布操作

- **README 产品截图**：新增 `view.png` 截图展示
- **画布点击失焦**：点击画布任意位置，右侧控件设置表单立即失焦（受控组件 value 已实时同步，不丢数据）
- **预览浮窗 resize**：JSONL / 语义预览浮窗支持右下角拖拽调整大小（最小尺寸 = 当前大小，即拖拽起点）
- **控件四边 resize**：除右下角手柄外，支持按住控件上下左右四边横向/纵向改尺寸（左/上边拖动时对侧锚定、最小尺寸与页面边界钳制、移动边对齐吸附与原行为一致）
- **多选外框四边 resize**：框选多选后的虚线外框支持上下左右四边批量调整——每个控件的调整幅度与外框移动量保持一致（线性，非等比）；四角等比缩放保留
- **框选稳定**：完全包含语义保持并由测试强化（含多选外框边手柄命中/角优先/空白回落）
- 验证：verify-interactions 98+ 断言全绿（新增四边 resize / 组四边 resize / 边命中决策 4 组）

## v2.1.0（2026）

### 存储介质 v2.1：每画布一个 JSON 文件，归类目录

- **画布正文介质从官方存储域单文件改为目录文件**：`~/.dsh/storages/wf-canvases/{canvasId}.json`（CanvasFile 完整形态，meta + elements 合一）；官方 json 后端固定单单位单文件平铺、无子目录、无单位枚举，无法满足「每画布一文件 + 目录归类」→ 正文介质由宿主半 node:fs 自管
- **原子写**：writeAtomic（临时文件 + fsync + rename，官方 JSON 后端同款模式）；崩溃安全保留
- **写放大消失**：单画布保存只重写自己那个文件（官方域单文件方案下任一画布保存触发整文件重写）
- **meta 缓存**：启动扫描 `wf-canvases/` 建内存索引（文件权威，损坏可重建）；无 100 条索引上限
- **容错**：损坏文件改名 `.corrupt` 隔离 + 缓存剔除；宿主侧写链串行（读-改-写）
- **迁移**：旧官方域单位文件 `wf_canvas.json` 启动自动拆分到每画布文件（只入不覆盖，成功后改名 `.migrated`）；v4 旧文件库迁移不变
- **传输与接口零改动**：官方 @Remote 网关（typert 严格路径 + remote.$mount）、wire 线协议、CanvasStore 接口、client 全部不变；`domainAdapter` 现役
- **依赖清理**：移除 `@deepseek-ai/dsh-storage-domain`（生产）与 `dsh-storage-json`（dev，冒烟改用真 fs 临时目录）
- 验证：verify-host-storage 35+ 断言（含域文件拆分迁移/只入不覆盖/损坏隔离）+ smoke-storage.mjs 真 fs 冒烟 + 10 套全绿

## v2.0.0（2026）

### 存储架构 v5 定稿：官方存储域 + @Remote 网关

- **存储内核迁移官方 `ctx.storageDomain`**：数据落 `~/.dsh/storages/wf_canvas.json`（JSON 后端原子写：临时文件 + fsync + rename），meta/body 两表 + global；`saveBody` 增量 patch 映射 `table.update()` 原子 RMW；单写链串行化
- **传输层迁移官方 api-gateway**：宿主半 `ctx.provide('wfStorage')` + `bindTypertRemote` + `ctx.typert.register`（严格描述符，zod 线协议校验）；客户端 `ctx.remote.$mount` 挂载 `remote.wfStorage`（公开运行时 API，无需改官方装配）
- **删除全部自建存储基础设施**：`lib/wf-storage.js`（自管文件库）、`/api/wf-storage` 路由、`src/core/storage/rpc.js`（同步 XHR 探测）、`hostFileAdapter`、shell 删除依赖、`~/.dsh/wf/config.json` 逻辑
- **`domainAdapter` 现役**：CanvasStore 接口与业务层（hooks/components）零改动；探测链 `domain > indexedDB > localStorage`
- **旧数据一次性迁移**：v4 文件库（`~/Documents/界面草图/dsh-wf/`）启动时自动迁移（只入不覆盖，成功改名 `.migrated`）
- **损坏容错升级**：官方 `malformed-medium` 拒绝 → `.corrupt` 隔离重开空库；修复 index 100 条隐藏上限
- 依赖：`@deepseek-ai/dsh-storage-domain` / `dsh-typert-protocol` / `zod`（版本与 DSH 运行时严格一致 rc.7）；`dsh-storage-json` 为 devDependency（真后端冒烟）
- 验证：10 套全绿 + `scripts/smoke-domain.mjs` 真后端冒烟（原子写单位文件 / 损坏隔离 / 级联删除）

## v1.1.0（2026-08）

### 设计哲学：结构陈述而非高保真

- **JSONL 字段收敛 17 → 6**：仅保留结构内容（`text` / `placeholder` / `inputType` / `options` / `action` / `checked`）；
  资源地址、尺寸、播放行为、默认值、进度等实现细节一律走 `description`，由模型结合上下文实现
- `radio` 回归「单选项」语义（与 `checkbox` 对称）：互斥单选组由容器结构表达，`options` 不再用于单选
- checkbox/radio 的标签并入 `text`（JSONL 输出 `props.text`，面板显示「标签文字」）

### 画布样式体系重构

- 统一填充体系（静态区 / 强调区 / 无背景类），移除硬编码黑色
- 占位符全部几何化（图片图标、音频波形、四角星、对勾 path、下拉箭头、链环），无文字/字形占位
- checkbox/radio/switch 增加弱虚线外框圈定组件大小；progress 固定示意填充
- 每类型最小尺寸注册表化（minW/minH），resize/组缩放统一钳制

### 业务管线重构（新建画布）

- 串行持久化队列 + 全量快照保存 + 显式创建画布 id：
  修复「新建后列表不显示」「幽灵画布」「切换后页面丢失」等并发竞态问题
- 新建严格顺序：旧画布落盘 → 新文件落盘（列表刷新）→ 选中 → 展示，防连点
- 宿主存储 saveMeta 加互斥锁（防御 index 并发覆盖）

### 交互与界面

- 模式：**长按 `Alt` 临时进入绘制模式，松开恢复选择**（原「控件模式」更名「绘制模式」）
- 空格按住 = 平移画布（含控件上方），ref 实时读取消除滞后，CSS 强制手型光标
- 复制/粘贴：支持多选与容器/页面连带子元素，整体偏移防重叠
- resize 下边/右边对齐吸附（阈值与移动一致），吸附虚线反馈
- 右侧面板：控件设置/画布历史模块化；历史高度拖拽（title 栏上边触发）；列表按创建日期排序、仅显示日期；hover 切换日期↔操作按钮（固定行高防抖动）
- 双击编辑：点击画布其他位置自动保存并关闭编辑框；Esc 关闭
- 启动恢复最近打开的画布（宿主存储异步恢复）；新建画布立即落盘
- 历史列表操作按钮不再预留空间，hover 时隐藏日期显示按钮

### 验证

- 验证套件 9 → 10（新增新建画布端到端管线套件）

## v1.0.0（2025）

- 会话输入框界面草图插件首发：绘制草图 → JSONL 语义描述 → 嵌入输入框
- 七范式架构、注册表驱动（18 控件类型）、宿主文件存储、语义预览
